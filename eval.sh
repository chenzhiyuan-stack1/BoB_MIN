...
docker run --rm -v `pwd`:/app -w /app/environment/metrics --name eval challenge-env python3 eval_network.py --dst_network_log /app/webrtc.log  --output /app/out_eval_network.json --ground_recv_rate ${GROUND_RECV_RATE} --max_delay 2000
docker run --rm -v `pwd`:/app -w /app/environment/metrics --name eval challenge-env python3 eval_video.py --src_video /app/testmedia/test.yuv --dst_video /app/outvideo.yuv --frame_align_method ocr --output /app/out_eval_video.json
