...
runTestsOnModel() {
    ...
    # 修改 sender_pyinfer_iperf.json 的 video_file 和 audio_file 配置
    jq --arg vpath "$video_path" --argjson height $height --argjson width $width --argjson fps $fps \
      ...
      sender_pyinfer_iperf.json > sender_pyinfer_tmp.json && mv sender_pyinfer_tmp.json sender_pyinfer_online.json
